...
export function WorkRequestTabStepper({
    ...
    const safeMaxEnabled = Math.min(maxEnabledIndex, Math.max(0, totalSteps - 1));

    const currentTitle = steps[safeCurrent]?.title ?? '';
    const progress = totalSteps > 0 ? Math.round(((safeCurrent + 1) / totalSteps) * 100) : 0;
            ...
            <nav aria-label="Work request steps" className="hidden sm:block">
                <ol role="tablist" className="flex items-center">
                    {steps.map((step, index) => {
                        ...
                        const isValid = allValid ? true : index < safeMaxEnabled;
                        const isTarget = !allValid && index === safeMaxEnabled;

                        const connectorClass = cn(
                            'absolute left-0 top-[18px] z-0 h-0.5 w-full',
                            index < safeMaxEnabled ? 'bg-blue-600' : 'bg-slate-200',
                        );

                        const circleClass = cn(
                            'flex h-9 w-9 items-center justify-center rounded-full border text-sm font-semibold transition-colors',
                            !isEnabled && 'border-slate-300 bg-white text-slate-400',
                            isValid && 'border-blue-600 bg-blue-600 text-white',
                            isTarget && 'border-blue-600 bg-white text-blue-600',
                            isCurrent && isValid && 'ring-2 ring-blue-600 ring-offset-2',
                            isCurrent && isTarget && !hasError && 'ring-2 ring-blue-600 ring-offset-2',
                            isCurrent && isTarget && hasError && 'border-red-500 text-red-600 ring-2 ring-red-500 ring-offset-2',
                        );
                        ...
                        return (
                            <li key={step.id} className="relative flex-1">
                                {index !== totalSteps - 1 && <div aria-hidden="true" className={connectorClass} />}
                                ...
                                >
                                    <span className={circleClass}>
                                        {isValid ? <Check className="h-4 w-4" /> : index + 1}
                                    </span>
